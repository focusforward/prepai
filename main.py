...
The question must be specific to {topic} within {subject} — not a generic question."""
        ...
        print(f"❌ generate-question error: {tb}")
        return {"error": f"Failed to generate question: {str(e)}", "detail": tb}
...
- The expected output (if test cases were provided)

Evaluate on:
1. CORRECTNESS (40%): Does the code produce the right output? Does logic work?
...
Return ONLY valid JSON:
{
  ...
  "correctness": "<did the code work? what was wrong if not?>",
...
async def run_code(request: Request):
    """Execute student code via Piston API and evaluate with AI"""
    try:
        data = await request.json()
        ...
        async with httpx.AsyncClient(timeout=15.0) as client_http:
            piston_res = await client_http.post(PISTON_URL, json={
        ...
        actual_output = run.get("stdout", "").strip()
        ...
        if stderr and not actual_output:
            ...
            execution_result = actual_output or "(no output)"
            has_error = False
...
Exit Code: {exit_code}"""
        ...
        return {"error": "Code execution timed out. Check for infinite loops."}
    except Exception as e:
