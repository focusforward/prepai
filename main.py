...
APTITUDE_PATH = BASE_DIR / "aptitude_questions.xlsx"
...
def get_aptitude_categories():
    """Return all aptitude categories"""
    global APTITUDE_DATA
    if APTITUDE_DATA is None:
        load_aptitude()
    if APTITUDE_DATA is None:
        return {"error": "Aptitude questions not loaded"}
    categories = sorted(APTITUDE_DATA['Category'].unique().tolist())
...
async def generate_aptitude(request: Request):
    ...
    if len(filtered) == 0:
        return {"error": "No questions in this category"}

    random_q = filtered.sample(n=1).iloc[0]

    return {
        "category": random_q['Category'],
        "subcategory": random_q['Subcategory'],
        "question": random_q['Question'],
        "options": {
            "a": str(random_q['OptionA']),
            "b": str(random_q['OptionB']),
            "c": str(random_q['OptionC']),
            "d": str(random_q['OptionD'])
        },
        "correct_answer": str(random_q['Answer']).strip().lower(),
