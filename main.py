...
# -------------------- AUTH / DB --------------------
DB_PATH = Path("/data/prepto_users.db") if Path("/data").exists() else BASE_DIR / "prepto_users.db"
JWT_SECRET = os.getenv("JWT_SECRET", secrets.token_hex(32))

TIER_LIMITS = {
    "free":          {"daily_questions": 10, "coding": False, "diagrams": False},
    "paid":          {"daily_questions": 999, "coding": True,  "diagrams": True},
    "institutional": {"daily_questions": 999, "coding": True,  "diagrams": True},
    "admin":         {"daily_questions": 999, "coding": True,  "diagrams": True},
}
...
def verify_password(password: str, stored: str) -> bool:
    try:
        ...
        h = hashlib.pbkdf2_hmac("sha256", password.encode(), salt.encode(), 100000)
        return hmac.compare_digest(base64.b64encode(h).decode(), hashed)
    except Exception:
        return False

def create_token(user_id: int, email: str, tier: str) -> str:
    payload = f"{user_id}:{email}:{tier}:{int(time.time()) + 86400 * 30}"
...
def verify_token(token: str):
    try:
        ...
        if not hmac.compare_digest(sig, expected):
            return None
        parts = payload.split(":")
        user_id, email, tier, expires = int(parts[0]), parts[1], parts[2], int(parts[3])
...
def check_usage(user_id: int, tier: str) -> dict:
    ...
    conn.close()
    daily_limit = limit["daily_questions"]
    return {
        "used": used,
        "limit": daily_limit,
        "remaining": max(0, daily_limit - used),
        "exceeded": used >= daily_limit,
        "coding_allowed": limit["coding"],
        "diagrams_allowed": limit["diagrams"],
    }
...
async def signup(request: Request):
    try:
        ...
        try:
            conn.execute(
                "INSERT INTO users (email, name, password, tier, college) VALUES (?, ?, ?, 'free', ?)",
                (email, name, hashed, college or None)
            )
            conn.commit()
        ...
        return {"token": token, "user": {"email": email, "name": name, "tier": "free"}}
...
async def login(request: Request):
    try:
        ...
        usage = check_usage(user["id"], user["tier"])
        return {
            "token": token,
            "user": {
                "email": email,
...
async def me(authorization: Optional[str] = Header(None)):
    ...
    usage = check_usage(user["user_id"], user["tier"])
    return {
        "email": row["email"],
        "name":  row["name"],
        "tier":  row["tier"],
...
@app.post("/auth/check-usage")
async def check_usage_endpoint(request: Request, authorization: Optional[str] = Header(None)):
    user = get_current_user(authorization)
    if not user:
        # Unauthenticated users treated as free with no daily tracking
        return {"tier": "guest", "exceeded": False, "remaining": 3, "used": 0, "limit": 3,
                "coding_allowed": False, "diagrams_allowed": False}
    usage = check_usage(user["user_id"], user["tier"])
    return {"tier": user["tier"], **usage}

@app.post("/auth/increment")
async def increment_endpoint(authorization: Optional[str] = Header(None)):
    user = get_current_user(authorization)
    if user:
...
async def admin_users(authorization: Optional[str] = Header(None)):
    ...
    conn = get_db()
    users = conn.execute("SELECT id, email, name, tier, college, created_at FROM users ORDER BY id DESC").fetchall()
    conn.close()
    return {"users": [dict(u) for u in users]}

@app.post("/admin/set-tier")
async def admin_set_tier(request: Request, authorization: Optional[str] = Header(None)):
    user = get_current_user(authorization)
    if not user or user["tier"] != "admin":
        return JSONResponse({"error": "Admin access required"}, status_code=403)
    data = await request.json()
    ...
    if not affected:
        return JSONResponse({"error": "User not found"}, status_code=404)
    return {"ok": True, "email": target_email, "tier": new_tier}

@app.post("/admin/create-admin")
async def create_admin(request: Request):
    """One-time endpoint to create first admin. Disable after use."""
    data = await request.json()
    secret = data.get("secret", "")
    if secret != os.getenv("ADMIN_SECRET", "prepto_admin_2025"):
    ...
    try:
        conn.execute(
            "INSERT INTO users (email, name, password, tier) VALUES (?, ?, ?, 'admin')",
            (email, name, hashed)
        )
        conn.commit()
    except sqlite3.IntegrityError:
        conn.execute("UPDATE users SET tier='admin', password=? WHERE email=?", (hashed, email))
        conn.commit()
    conn.close()
    return {"ok": True, "message": f"Admin {email} created/updated"}
...
- Structure the ideal_answer in THREE clearly labeled sections:
  1. OPEN WITH: One powerful sentence that defines the concept precisely — the kind of opener that makes an interviewer nod immediately.
  2. EXPLAIN: The full technical depth — formulae, derivation, worked example, comparisons, boundary conditions. This is the body.
  3. CLOSE WITH: One memorable sentence — a real-world application, industry use case, or advanced insight that shows the candidate thinks beyond textbooks.
- Label each section exactly as: "OPEN WITH:", "EXPLAIN:", "CLOSE WITH:" on their own lines.
...
- Structure the ideal_answer in THREE clearly labeled sections:
  1. OPEN WITH: One powerful sentence that defines the concept precisely — the kind of opener that makes an interviewer nod immediately.
  2. EXPLAIN: The full technical depth — formulae, derivation, worked example, comparisons, boundary conditions. Written in flowing paragraphs, no bullet points.
  3. CLOSE WITH: One memorable sentence — a real-world application, industry use case, or advanced insight that shows the candidate thinks beyond textbooks.
- Label each section exactly as: "OPEN WITH:", "EXPLAIN:", "CLOSE WITH:" on their own lines.
...
# Aptitude AI generation prompt for adaptive difficulty
APTITUDE_AI_PROMPT = """You are an expert aptitude test designer for Indian engineering placement exams (TCS, Infosys, Wipro, Cognizant, Accenture, etc.).

Generate ONE multiple-choice aptitude question at the specified difficulty level.

Difficulty guidelines:
- Easy: Basic formula application, single-step problems, common vocab/grammar
- Medium: 2-3 step problems, moderate complexity, inference-based verbal
- Hard: Multi-step reasoning, tricky conditions, advanced verbal/logical patterns
...
async def generate_aptitude(request: Request):
    try:
        ...
        difficulty = data.get("difficulty", None)  # Easy / Medium / Hard

        # Try Excel first
        use_ai = False
        if APTITUDE_DATA is not None:
            if category:
            ...
            # Filter by difficulty if Excel has a Difficulty column
            cols_lower = {c.strip().lower().replace(" ", ""): c for c in APTITUDE_DATA.columns}
            if difficulty and "difficulty" in cols_lower:
                diff_col = cols_lower["difficulty"]
                diff_filtered = filtered[filtered[diff_col].str.strip().str.lower() == difficulty.lower()]
        ...
        # AI generation fallback (used when difficulty filter needed or no Excel)
        type_hint = ""
        if type_filter == "verbal":
            type_hint = "Generate a VERBAL reasoning question (grammar, reading comprehension, vocabulary, sentence correction, or critical reasoning)."
        elif type_filter == "quantitative":
            type_hint = "Generate a QUANTITATIVE aptitude question (arithmetic, algebra, geometry, time-speed-distance, percentages, permutation-combination, or data interpretation)."
        elif category:
            type_hint = f"Generate a question from the category: {category}."
        else:
            type_hint = "Generate either a quantitative or verbal aptitude question."

        prompt = f"{type_hint} Difficulty: {difficulty or 'Medium'}."

        ai_res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": APTITUDE_AI_PROMPT},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            response_format={"type": "json_object"}
...
code_eval_prompt = """
You are a senior software engineer reviewing a student's coding interview answer.

You will receive:
- The coding question
- The language used
- The student's code
- The actual output when the code was run (or an error message)

Evaluate on:
1. CORRECTNESS (40%): Does the code produce the right output?
2. CODE QUALITY (30%): Is it clean, readable, well-structured?
3. EFFICIENCY (20%): Is the time/space complexity reasonable?
4. INTERVIEW READINESS (10%): Would this impress an interviewer?

CRITICAL RULES:
- "optimized_solution" is MANDATORY. You MUST always provide a complete, working optimal solution in the same language with inline comments. Never return null, empty string, or "N/A" for this field. Even if the student's code is perfect, show the cleanest possible version.
- "efficiency" MUST state the exact Big O time and space complexity (e.g. "O(n) time, O(1) space").
...
Return ONLY valid JSON:
{
  ...
  "what_you_got_right": "<specific praise>",
  "what_to_fix": "<specific issues to correct>",
  "efficiency": "<exact Big O time and space complexity>",
  "optimized_solution": "<MANDATORY: complete working optimal solution in the same language with inline comments explaining each key step>",
  "interviewer_verdict": "<would you hire? what one thing should they fix?>"
}
"""
...
async def run_code(request: Request):
    try:
        ...
        lang_config = PISTON_LANGUAGE_MAP.get(language)
        if not lang_config:
        ...
        # For Python: run directly via subprocess (fast, reliable)
        if language == "python":
            ...
            try:
                ...
                output = output or stderr_out or "No output"
            ...
            try:
                async with httpx.AsyncClient(timeout=15.0) as http:
                    piston_res = await http.post(PISTON_URL, json={
                        "language": lang_config["language"],
                        "version": lang_config["version"],
                        "files": [{"content": code}]
                    })
                    piston_data = piston_res.json()

                run = piston_data.get("run", {})
                stdout = run.get("stdout", "")
                stderr_out = run.get("stderr", "")
                output = stdout or stderr_out or "No output"
            except Exception:
                output = "Code execution unavailable for this language. Submit & Evaluate still works!"

        if not evaluate:
            return {"output": output, "stderr": stderr_out}

        # AI evaluation
        eval_prompt = f"""Coding Question: {question}
Language: {language}
Student's Code:
{code}
...
{output}"""
        ...
        ai_res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": code_eval_prompt},
...
def load_coding_db():
    global CODING_DB
    try:
        coding_path = BASE_DIR / "coding_questions.json"
        with open(coding_path, 'r') as f:
            CODING_DB = json.load(f)
        print(f"✅ Loaded {len(CODING_DB)} coding problems")
    except Exception as e:
        print(f"❌ Failed to load coding DB: {e}")
        CODING_DB = []

@app.get("/get-coding-topics")
def get_coding_topics():
    global CODING_DB
    if CODING_DB is None:
        load_coding_db()
    topics = sorted(list(set(p["topic"] for p in CODING_DB)))
    companies = sorted(list(set(c for p in CODING_DB for c in p["companies"])))
    return {"topics": topics, "companies": companies}

@app.get("/get-coding-question")
def get_coding_question(topic: str = "Any", difficulty: str = "Any", company: str = "Any"):
    global CODING_DB
    if CODING_DB is None:
        load_coding_db()
    if not CODING_DB:
        return {"error": "Coding database not loaded"}
    ...
    if company != "Any":
        pool = [p for p in pool if company in p["companies"]]
    
    if not pool:
        return {"error": f"No problems found for {topic} / {difficulty} / {company}"}
    
    problem = _random.choice(pool)
    return problem
...
PLANS = {
    "monthly": {"amount": 19900,  "currency": "INR", "desc": "Prepto Pro — Monthly (₹199/month)"},
    "yearly":  {"amount": 199900, "currency": "INR", "desc": "Prepto Pro — Yearly (₹1999/year)"},
}

def razorpay_api(method, path, body=None):
    ...
    request = _req.Request(url, method=method, headers=headers, data=data)
    try:
...
async def create_order(request: Request, authorization: Optional[str] = Header(None)):
    ...
    p = PLANS[plan]
    order, err = razorpay_api("POST", "/orders", {
        "amount": p["amount"],
        "currency": p["currency"],
        "receipt": f"prepto_{user['user_id']}_{plan}",
        "notes": {"user_id": str(user["user_id"]), "plan": plan}
    })
    if err or not order:
        return JSONResponse({"error": f"Could not create order: {err}"}, status_code=500)
    return {
        "order_id": order["id"],
        "amount":   order["amount"],
...
async def verify_payment(request: Request, authorization: Optional[str] = Header(None)):
    ...
    if not hmac.compare_digest(expected_sig, signature):
        return JSONResponse({"error": "Payment verification failed — invalid signature"}, status_code=400)
    # Upgrade user
    conn = get_db()
    conn.execute("UPDATE users SET tier='paid' WHERE id=?", (user["user_id"],))
    conn.commit()
    conn.close()
    # Issue fresh token with new tier
    new_token = create_token(user["user_id"], user["email"], "paid")
    return {"ok": True, "token": new_token, "tier": "paid", "plan": plan}
